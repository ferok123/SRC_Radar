...
#%% CA-CFAR Parameters
num_guard_cells = 15  # Número de celdas de guarda a cada lado
num_ref_cells = 100   # Número de celdas de referencia a cada lado
alpha = 3.85          # Factor de escala para la detección

# Inicializar la matriz de detección CFAR
detection_cfar = np.zeros_like(compressed_mti_signal)

# Recorrer cada fila de la señal MTI comprimida
for row in range(Np):
    signal_row = np.abs(compressed_mti_signal[row])
    num_cells = len(signal_row)
    cfar_threshold = np.zeros(num_cells)
    
    # Iterar a través de las celdas de la fila
    for i in range(num_ref_cells + num_guard_cells, num_cells - num_ref_cells - num_guard_cells):
        # Celdas de referencia antes y después de la celda de interés (CUT)
        ref_cells_before = signal_row[i - num_ref_cells - num_guard_cells:i - num_guard_cells]
        ref_cells_after = signal_row[i + num_guard_cells + 1:i + num_guard_cells + 1 + num_ref_cells]
        
        # Estimar el ruido como el promedio de las celdas de referencia
        noise_est = np.mean(np.concatenate((ref_cells_before, ref_cells_after)))
        
        # Calcular el umbral CFAR
        cfar_threshold[i] = alpha * noise_est
        
        # Detectar si la señal supera el umbral
        if signal_row[i] > cfar_threshold[i]:
            detection_cfar[row, i] = signal_row[i]
...
detection = np.zeros_like(compressed_mti_signal)
for row in range(Np):
    detection[row] = np.where(np.abs(compressed_mti_signal[row]) > MTI_threshold[row], 
                              compressed_mti_signal[row], 0)

# Detectar si la señal supera el umbral
if signal_row[i] > MTI_threshold[i]:
    detection_cfar[row, i] = signal_row[i]
...
ax = axes[3]
ax.plot(ranks/1e3, np.abs(detection_cfar[1]))
ax.set_ylabel('Abs value')
